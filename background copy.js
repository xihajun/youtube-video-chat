...
async function handleApiRequest(apiType, apiKey, apiModel, messageArray) {
    ...
    if (apiType === 'openai') {
        ...
        let conversation = messageArray.map(msg => `${msg.role === 'user' ? 'Human:' : 'Assistant:'}: ${msg.content}`).join('\n');
...
chrome.runtime.onMessage.addListener(async function (request) {
    if (request.input) {
        ...
        chrome.storage.local.get(['apiType', 'apiKey', 'apiModel'], async ({ apiType, apiKey, apiModel }) => {
            try {
                // Call the API
                let responseData = await handleApiRequest(apiType, apiKey, apiModel, messageArray);
